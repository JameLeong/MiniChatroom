...
public class WebServlet extends HttpServlet {
			...
			if (name.startsWith("Bean_")) {
				...
				try {
					if (beanClass == null || "".equals(beanClass.trim()))
						throw new Exception("未配置类名！-->" + beanName);
					// 反射得到类talkBean
					...
					try {
						throw new NoSuchFieldException("未找到Bean声明字段（"
								+ beanName + "）");
	...
	public final void service(HttpServletRequest request,
		...
		try {
			...
			action = this.get(
					(routeParam == null || "".equals(routeParam)) ? "action"
							: routeParam, "initPage");
	...
	protected void show(String page) {
		...
		try {
			request.getRequestDispatcher(
					((pagePath == null || "".equals(pagePath)) ? "/WEB-INF/pages/"
							: pagePath)
